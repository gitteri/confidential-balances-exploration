...
//!
//! Shared by the examples and demo flows so the legacy-POD → 6.0.1 byte casts
//! and lo/hi pending recombination live in exactly one place.
...
/// Cast an on-chain (4.0) POD ElGamal ciphertext to 6.0.1 and decode it.
fn decode_ciphertext<T: bytemuck::Pod>(field: &T, what: &str) -> CtResult<ElGamalCiphertext> {
    let pod = PodElGamalCiphertextV6(
        bytemuck::bytes_of(field)
            .try_into()
            .map_err(|_| format!("{what} size"))?,
    );
    pod.try_into().map_err(|e| format!("decode {what}: {e:?}").into())
}
...
pub fn read_balances(client: &RpcClient, owner: &dyn Signer, mint: &Pubkey) -> CtResult<Balances> {
    ...
    let avail_bytes: [u8; 36] = bytemuck::bytes_of(&ext.decryptable_available_balance)
        .try_into()
        .map_err(|_| "decryptable_available_balance size")?;
    let available = AeCiphertext::from_bytes(&avail_bytes)
