//! Batched confidential transfers from a *single* sender (bypass mode).
//!
...
use crate::transfer::{
    cast_ae_ciphertext_legacy_to_v6, cast_ae_ciphertext_v6_to_legacy,
    cast_elgamal_ciphertext_legacy_to_v6, cast_elgamal_ciphertext_v6_to_legacy,
    cast_elgamal_pubkey_legacy_to_v6, send_tx, stage_range_proof_record, RECORD_PROOF_OFFSET,
    ZK_PROOF_PROGRAM_ID,
};
use crate::types::*;
...
struct PreparedLeg {
    ...
    new_decryptable_legacy: PodAeCiphertextLegacy,
    auditor_lo_legacy: PodElGamalCiphertextLegacy,
    auditor_hi_legacy: PodElGamalCiphertextLegacy,
}

/// The four proof context / record accounts staged on chain for one leg.
struct StagedLeg {
...
/// Read the mint's optional auditor ElGamal pubkey (legacy POD → 6.0.1).
fn fetch_auditor_pubkey(client: &RpcClient, mint: &Pubkey) -> CtResult<Option<ElGamalPubkey>> {
    ...
    pod_opt
        .map(|pod| -> CtResult<ElGamalPubkey> {
            cast_elgamal_pubkey_legacy_to_v6(&pod)?
                .try_into()
...
/// Read a configured account's ElGamal pubkey (legacy POD → 6.0.1).
fn fetch_elgamal_pubkey(client: &RpcClient, token_account: &Pubkey) -> CtResult<ElGamalPubkey> {
    ...
    cast_elgamal_pubkey_legacy_to_v6(&ext.elgamal_pubkey)?
        .try_into()
...
fn prepare_legs(
    ...
    let mut current_ct: ElGamalCiphertext =
        cast_elgamal_ciphertext_legacy_to_v6(&sender_ext.available_balance)?
            .try_into()
            .map_err(|e| format!("sender available balance: {e:?}"))?;
    let current_decryptable = cast_ae_ciphertext_legacy_to_v6(&sender_ext.decryptable_available_balance)?;
    ...
    for leg in legs {
        ...
        let new_decryptable_legacy = cast_ae_ciphertext_v6_to_legacy(&sender_aes.encrypt(running));
        let auditor_lo_legacy = cast_elgamal_ciphertext_v6_to_legacy(
            &proof.ciphertext_validity_proof_data_with_ciphertext.ciphertext_lo,
        );
        let auditor_hi_legacy = cast_elgamal_ciphertext_v6_to_legacy(
            &proof.ciphertext_validity_proof_data_with_ciphertext.ciphertext_hi,
        );
...
/// Pre-verify a single leg's three proofs into context state accounts. The
/// oversized U128 range proof is staged through an spl-record account and
/// verified from there, mirroring the single-transfer flow.
fn stage_leg(
    client: &RpcClient,
    payer: &dyn Signer,
    authority: &Pubkey,
    ...
    let equality_verify_ix = ProofInstruction::VerifyCiphertextCommitmentEquality
        .encode_verify_proof(
            Some(ContextStateInfo {
                ...
                context_state_authority: &Address::from(authority.to_bytes()),
            }),
    ...
    let validity_verify_ix = ProofInstruction::VerifyBatchedGroupedCiphertext3HandlesValidity
        .encode_verify_proof(
            ...
            &leg.proof.ciphertext_validity_proof_data_with_ciphertext.proof_data,
        );
    sigs.push(send_tx(
        client,
    ...
    // 3. Range proof, staged through a record account then verified from it.
...
fn transfer_ix_for(
    ...
    inner_transfer(
        ...
        &leg.new_decryptable_legacy,
        &leg.auditor_lo_legacy,
        &leg.auditor_hi_legacy,
        authority,
        &[],
        equality_loc,
        validity_loc,
...
/// Close a leg's proof context accounts and its record account. Reclaims rent
/// to `payer`.
fn close_leg_ixs(payer: &Pubkey, authority: &Pubkey, staged: &StagedLeg) -> Vec<Instruction> {
    let payer_addr = Address::from(payer.to_bytes());
    let authority_addr = Address::from(authority.to_bytes());
    ...
    vec![
        ...
        spl_record::instruction::close_account(&staged.record_account, payer, payer),
    ]
}
...
pub async fn batch_transfer_pipelined(
    ...
    for (i, leg) in prepared.iter().enumerate() {
        let staged = stage_leg(client, payer, &sender.pubkey(), leg, &mut sigs)?;

        let transfer_ix =
            transfer_ix_for(&sender_token_account, mint, &sender.pubkey(), leg, &staged)?;
        let mut ixs = vec![transfer_ix];
        ixs.extend(close_leg_ixs(&payer.pubkey(), &sender.pubkey(), &staged));
...
pub async fn batch_transfer_atomic(
    ...
    let alt = AddressLookupTableAccount {
        key: Address::from(lut_address.to_bytes()),
        addresses: lut_addresses
            .iter()
            .map(|p| Address::from(p.to_bytes()))
            .collect(),
    };
    ...
    // Reclaim rent: close every leg's proof context + record accounts. The
    // transfers already consumed the verified proofs.
    for staged in &staged_legs {
        let ixs = close_leg_ixs(&payer.pubkey(), &sender.pubkey(), staged);
        sigs.push(send_tx(client, &ixs, &[payer, sender], &payer.pubkey())?);
    }
